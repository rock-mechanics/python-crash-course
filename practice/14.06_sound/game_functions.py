...
def check_events(ship, bullets, settings, stats, button, screen, aliens, score) : 
    ...
    for event in pygame.event.get() : 
        if event.type == pygame.QUIT : 
            ...
            check_play_button(stats, button, mouse_x, mouse_y, screen, aliens, bullets, settings, ship, score)

def check_play_button(stats, button, mouse_x, mouse_y, screen, aliens, bullets, settings, ship, score) : 
    ...
    if button_clicked and not stats.game_active : 
        ...
        start_game(aliens, bullets, settings, score, stats, screen, ship) 
...
def check_bullet_alien_collision(bullets, aliens, settings, screen, ship, score, stats) : 
    ...
    if collisions : 
        # play the sound
        threading.Thread(target=play_boom, daemon=True).start()
    ...
    # all aliens are shot
    if len(aliens) == 0 : 
        start_game(aliens, bullets, settings, score, stats, screen, ship, True)

def start_game(aliens, bullets, settings, score, stats, screen, ship, nextlevel = False) : 
    if nextlevel : 
        # previous level cleared, go to next level
        stats.level += 1
...
def update_aliens(aliens, settings, ship, stats, screen, bullets, score) : 
    ...
    check_aliens_bottom(settings, stats, screen, ship, aliens, bullets, score)

def ship_hit(settings, stats, screen, ship, aliens, bullets, score) : 
...
def check_aliens_bottom(settings, stats, screen, ship, aliens, bullets, score) : 
